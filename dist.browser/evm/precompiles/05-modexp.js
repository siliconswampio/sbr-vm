...
function getAdjustedExponentLength(data) {
    var expBytesStart;
    try {
        var baseLen = new sbr_util_1.BN(data.slice(0, 32)).toNumber();
    ...
    var expLen = new sbr_util_1.BN(data.slice(32, 64));
    ...
    var firstExpBN = new sbr_util_1.BN(firstExpBytes);
    var max32expLen = 0;
    if (expLen.ltn(32)) {
...
function default_1(opts) {
    ...
    var bLen = new sbr_util_1.BN(data.slice(0, 32));
    var eLen = new sbr_util_1.BN(data.slice(32, 64));
    var mLen = new sbr_util_1.BN(data.slice(64, 96));
    var maxLen = bLen;
    if (maxLen.lt(mLen)) {
        maxLen = mLen;
    }
    ...
    var gasUsed;
    var bStart = new sbr_util_1.BN(96);
    ...
    var maxInt = new sbr_util_1.BN(Number.MAX_SAFE_INTEGER);
    var maxSize = new sbr_util_1.BN(2147483647); // sbr-util setLengthRight limitation
    if (bLen.gt(maxSize) || eLen.gt(maxSize) || mLen.gt(maxSize)) {
        return evm_1.OOGResult(opts.gasLimit);
    }
    var B = new sbr_util_1.BN(sbr_util_1.setLengthRight(data.slice(bStart.toNumber(), bEnd.toNumber()), bLen.toNumber()));
    var E = new sbr_util_1.BN(sbr_util_1.setLengthRight(data.slice(eStart.toNumber(), eEnd.toNumber()), eLen.toNumber()));
    var M = new sbr_util_1.BN(sbr_util_1.setLengthRight(data.slice(mStart.toNumber(), mEnd.toNumber()), mLen.toNumber()));
    if (mEnd.gt(maxInt)) {
        return evm_1.OOGResult(opts.gasLimit);
    }
    var R;
