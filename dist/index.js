...
class VM extends AsyncEventEmitter {
    ...
    constructor(opts = {}) {
        ...
        if (opts.stateManager) {
            ...
            const trie = opts.state || new sbr_merkle_patricia_tree_1.SecureTrie();
    ...
    async init() {
        ...
        if (this._opts.activatePrecompiles && !this._opts.stateManager) {
            ...
            await Promise.all(Object.keys(precompiles_1.precompiles)
                .map((k) => new sbr_util_1.Address(Buffer.from(k, 'hex')))
                .map(async (address) => {
                const account = sbr_util_1.Account.fromAccountData({ balance: 1 });
