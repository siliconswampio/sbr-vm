...
async function default_1(opts) {
    ...
    const baseGas = new sbr_util_1.BN(opts._common.paramByEIP('gasPrices', 'Bls12381PairingBaseGas', 2537));
    if (inputData.length == 0) {
        return evm_1.VmErrorResult(new exceptions_1.VmError(exceptions_1.ERROR.BLS_12_381_INPUT_EMPTY), opts.gasLimit);
    }
    const gasUsedPerPair = new sbr_util_1.BN(opts._common.paramByEIP('gasPrices', 'Bls12381PairingPerPairGas', 2537));
    const gasUsed = baseGas.iadd(gasUsedPerPair.imul(new sbr_util_1.BN(Math.floor(inputData.length / 384))));
