...
const fieldModulus = new sbr_util_1.BN('1a0111ea397fe69a4b1ba7b6434bacd764774b84f38512bf6730d2a0f6b0f6241eabfffeb153ffffb9feffffffffaaab', 16);
...
function BLS12_381_FromG1Point(input) {
    ...
    // note: decoded[0] == 1
    const xval = sbr_util_1.padToEven(decoded[1]);
    const yval = sbr_util_1.padToEven(decoded[2]);
    // convert to buffers.
...
function BLS12_381_FromG2Point(input) {
    ...
    const decoded = decodeStr.match(/"?[0-9a-f]+"?/g); // match above pattern.
    // note: decoded[0] == 1
    const x_1 = sbr_util_1.padToEven(decoded[1]);
    const x_2 = sbr_util_1.padToEven(decoded[2]);
    const y_1 = sbr_util_1.padToEven(decoded[3]);
    const y_2 = sbr_util_1.padToEven(decoded[4]);
    // convert to buffers.
...
function BLS12_381_ToFpPoint(fpCoordinate, mcl) {
    ...
    if (new sbr_util_1.BN(fpCoordinate).gte(fieldModulus)) {
...
function BLS12_381_ToFp2Point(fpXCoordinate, fpYCoordinate, mcl) {
    ...
    if (new sbr_util_1.BN(fpXCoordinate).gte(fieldModulus)) {
        throw new exceptions_1.VmError(exceptions_1.ERROR.BLS_12_381_FP_NOT_IN_FIELD);
    }
    if (new sbr_util_1.BN(fpYCoordinate).gte(fieldModulus)) {
        throw new exceptions_1.VmError(exceptions_1.ERROR.BLS_12_381_FP_NOT_IN_FIELD);
    }
    const fp_x = new mcl.Fp();
    const fp_y = new mcl.Fp();
