...
function getAdjustedExponentLength(data) {
    let expBytesStart;
    try {
        const baseLen = new sbr_util_1.BN(data.slice(0, 32)).toNumber();
    ...
    const expLen = new sbr_util_1.BN(data.slice(32, 64));
    let firstExpBytes = Buffer.from(data.slice(expBytesStart, expBytesStart + 32)); // first word of the exponent data
    firstExpBytes = sbr_util_1.setLengthRight(firstExpBytes, 32); // reading past the data reads virtual zeros
    let firstExpBN = new sbr_util_1.BN(firstExpBytes);
    let max32expLen = 0;
    if (expLen.ltn(32)) {
    ...
    if (expLenMinus32OrZero.ltn(0)) {
        expLenMinus32OrZero = new sbr_util_1.BN(0);
    }
...
function expmod(B, E, M) {
    if (E.isZero())
        return new sbr_util_1.BN(1).mod(M);
    // Red asserts M > 1
    if (M.lten(1))
        return new sbr_util_1.BN(0);
    const red = sbr_util_1.BN.red(M);
...
function default_1(opts) {
    ...
    if (adjustedELen.ltn(1)) {
        adjustedELen = new sbr_util_1.BN(1);
    }
    const bLen = new sbr_util_1.BN(data.slice(0, 32));
    const eLen = new sbr_util_1.BN(data.slice(32, 64));
    const mLen = new sbr_util_1.BN(data.slice(64, 96));
    let maxLen = bLen;
    if (maxLen.lt(mLen)) {
        maxLen = mLen;
    }
    ...
    let gasUsed;
    const bStart = new sbr_util_1.BN(96);
    ...
    if (bLen.isZero()) {
        return {
            gasUsed,
            returnValue: new sbr_util_1.BN(0).toArrayLike(Buffer, 'be', mLen.toNumber()),
        };
    }
    if (mLen.isZero()) {
        return {
    ...
    const maxInt = new sbr_util_1.BN(Number.MAX_SAFE_INTEGER);
    const maxSize = new sbr_util_1.BN(2147483647); // sbr-util setLengthRight limitation
    if (bLen.gt(maxSize) || eLen.gt(maxSize) || mLen.gt(maxSize)) {
        return evm_1.OOGResult(opts.gasLimit);
    }
    const B = new sbr_util_1.BN(sbr_util_1.setLengthRight(data.slice(bStart.toNumber(), bEnd.toNumber()), bLen.toNumber()));
    const E = new sbr_util_1.BN(sbr_util_1.setLengthRight(data.slice(eStart.toNumber(), eEnd.toNumber()), eLen.toNumber()));
    const M = new sbr_util_1.BN(sbr_util_1.setLengthRight(data.slice(mStart.toNumber(), mEnd.toNumber()), mLen.toNumber()));
    if (mEnd.gt(maxInt)) {
        return evm_1.OOGResult(opts.gasLimit);
    }
    let R;
