...
async function runTx(opts) {
    ...
    if ('transactionType' in opts.tx &&
        ...
        castedTx.AccessListJSON.forEach((accessListItem) => {
            const address = sbr_util_1.toBuffer(accessListItem.address);
...
async function _runTx(opts) {
     ...
     * Execute message
     */
    const txContext = new txContext_1.default(tx.gasPrice, caller);
    ...
    if (results.execResult.selfdestruct) {
        ...
        for (const k of keys) {
            const address = new sbr_util_1.Address(Buffer.from(k, 'hex'));
 ...
 */
async function generateTxReceipt(tx, txResult, blockGasUsed) {
    var _a;
    const baseReceipt = {
